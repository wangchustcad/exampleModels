...
block NormalForceBlockVib
  ...
  Modelica.Blocks.Interfaces.RealInput vib
...
public
  RandomBlock randomBlock(samplePeriod=0.1)
...
//  h_vib := if r_rel < 0 then 0.0005 + 0.0005*vib*sin(50*time) else 0;

  h_vib := if r_rel_abs < 0 then (0.1 + vib) * (0.0005 * randomBlock.y +
           0.0005*sin(2*Modelica.Constants.pi*time)) else
           (0.1) * (0.0005 * randomBlock.y +
           0.0005*sin(2*Modelica.Constants.pi*time));
